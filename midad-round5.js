...
(() => {
  ...
  async function secureMaterialImages() {
    ...
    await Promise.all(items.map(async it => {
      ...
      try {
        ...
        if (!promise) {
          promise = sb.functions.invoke('storage-router', { body: { bucket:'materials', path:cleanPath, action:'view' } }).then(async routedResult => {
            const {data:routed,error:routeError}=routedResult || {};
            if (routeError) throw routeError;
            if (!routed?.success || !routed?.url) throw new Error(routed?.error || 'تعذر اختيار Storage للمشاهدة.');
            const res=await fetch(routed.url,{cache:'no-store'});
            if(!res.ok) throw new Error(`فشل عرض الملف من ${routed.storeName || routed.store}: HTTP ${res.status}`);
  ...
  // PDF fingerprinting / per-user prepared copies are DISABLED.
  // PDFs are downloaded directly from the shared materials bucket.
  async function downloadStorageBlob(bucket, path, name, meta={}) {
    ...
    let blob;
    if (bucket === 'materials') {
      const { data: routed, error: routeError } = await sb.functions.invoke('storage-router', { body: { bucket, path: cleanPath, action: 'download' } });
      if (routeError) throw routeError;
      if (!routed?.success || !routed?.url) throw new Error(routed?.error || 'تعذر اختيار Storage للتحميل.');
      const res = await fetch(routed.url, { cache: 'no-store' });
      if (!res.ok) throw new Error(`فشل تحميل الملف من ${routed.storeName || routed.store}: HTTP ${res.status}`);
      blob = await res.blob();
    } else {
      const result = await sb.storage.from(bucket).download(cleanPath);
  ...
  uploadFile = async function(bucket, file, folder='uploads') {
    ...
    const isMaterialAsset = bucket === 'materials' && (String(folder) === 'files' || String(folder) === 'images' || String(folder) === 'dlonly');
    if (!isMaterialAsset) return originalUploadFile(bucket, file, folder);

    const folderName = String(folder) === 'images' ? 'images' : (String(folder) === 'dlonly' ? 'dlonly' : 'files');
    ...
    // بعد نجاح الرفع في المشروع الأصلي، انسخ الكائن إلى مخازن النسخ
    // المفعّلة فقط. الدالة المشتركة تتولى إعادة المحاولة والتحقق من النتيجة.
    try {
      await replicateMaterialUpload(path, file);
    } catch (err) {
  ...
  saveModal = async function () {
    ...
    try {
      ...
      const dlonlyFileObj = m.dlonlyFile instanceof File ? m.dlonlyFile : null;
      let newImagePath = null;
      let newFilePath = null;
      let newDlonlyFilePath = null;
      ...
      if ((m.kind === 'item' || m.kind === 'add-item') && dlonlyFileObj) newDlonlyFilePath = await uploadFile('materials', dlonlyFileObj, 'dlonly');

      // زر العرض + التحكم اليدوي في download-only.html — القيم دي بتتحط في state.modal مباشرة من نافذة التعديل
      const buttonMode = m.buttonMode || 'both';
      const previewOverride = m.previewOverride === 'download_only' ? 'download_only' : 'auto';
      const dlonlySourceType = previewOverride === 'download_only' ? (m.dlonlySourceType || 'upload') : null;
      const dlonlyLinkVal = previewOverride === 'download_only' && dlonlySourceType === 'link' ? (m.dlonlyLink || null) : null;
      const dlonlyFilePathVal = previewOverride === 'download_only' && dlonlySourceType === 'upload' ? (newDlonlyFilePath || m.dlonlyFilePath || null) : null;
      const dlonlyDisplayNameVal = previewOverride === 'download_only' ? (m.dlonlyDisplayName || null) : null;
      const dlonlyTargetVal = previewOverride === 'download_only' ? (m.dlonlyTarget || 'both') : null;
      ...
      if (m.kind === 'add-section' || m.kind === 'section') {
        ...
        const payload = applyImage({
          ...
          dlonly_source_type: dlonlySourceType,
          dlonly_file_path: dlonlyFilePathVal,
          dlonly_link: dlonlyLinkVal,
          dlonly_display_name: dlonlyDisplayNameVal,
          dlonly_target: dlonlyTargetVal,
          ...extra
        }, m.imagePathRaw);
